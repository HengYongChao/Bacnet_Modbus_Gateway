...
#define GUDPMC_ENABLE_DEBUG_MSG		0 // 1: enable 0:disable
...
static U8_T gudpmc_Multicast[MAC_ADDRESS_LEN] = {1, 0, 0x5e, 1, 2, 3};
...
U8_T GUDPMC_NewConn(U32_T XDATA* pip, U16_T remotePort, U8_T socket)
{
	...
	for (i = 0; i < GUDPMC_MAX_CONNS; i++)
	{
		if (gudpmc_Conns[i].State != GUDPMC_STATE_FREE)
		   TCPIP_UdpClose(gudpmc_Conns[i].UdpSocket); //tony 2009-4-20
		gudpmc_Conns[i].State = GUDPMC_STATE_CONNECTED;
		gudpmc_Conns[i].UdpSocket = socket;

#if GUDPMC_ENABLE_DEBUG_MSG
	printd("GUDPMC_NewConn(id=%bu, remotePort=%d, socket=%bu)\n\r", i, remotePort, socket);
#endif
		return i;
	}
...
#if GUDPMC_ENABLE_DEBUG_MSG
	printd("GUDPMC_Receive(id=%bu, len=%d)...\n\r", id, length);
#endif
