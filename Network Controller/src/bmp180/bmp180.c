...
p_bmp180->BMP180_BUS_WRITE_FUNC = BMP180_I2C_bus_write;
...
int bmp180_init(struct bmp180_t *bmp180)
{
   ...
   comres += p_bmp180->BMP180_BUS_READ_FUNC(p_bmp180->dev_addr,\
		BMP180_CHIP_ID__REG, &vdata, 1);  /* read Chip Id */

   p_bmp180->chip_id = BMP180_GET_BITSLICE(vdata, BMP180_CHIP_ID);
   ...
   p_bmp180->sw_oss = 0;
	if (p_bmp180->chip_id == BMP180_CHIP_ID) {
		...
		/* read Version reg */
		comres += p_bmp180->BMP180_BUS_READ_FUNC(p_bmp180->dev_addr,\
		BMP180_VERSION_REG, &vdata, 1);

		/* get ML Version */
		p_bmp180->ml_version = BMP180_GET_BITSLICE(vdata,\
		BMP180_ML_VERSION);
		/* get AL Version */
		p_bmp180->al_version = BMP180_GET_BITSLICE(vdata,\
		BMP180_AL_VERSION);
...
{
   int comres;
   unsigned char vdata[22];
   comres = p_bmp180->BMP180_BUS_READ_FUNC(p_bmp180->dev_addr,\
	BMP180_PROM_START__ADDR, vdata, BMP180_PROM_DATA__LEN);

   /*parameters AC1-AC6*/
   p_bmp180->cal_param.ac1 =  (vdata[0] << 8) | vdata[1];
   p_bmp180->cal_param.ac2 =  (vdata[2] << 8) | vdata[3];
   p_bmp180->cal_param.ac3 =  (vdata[4] << 8) | vdata[5];
   p_bmp180->cal_param.ac4 =  (vdata[6] << 8) | vdata[7];
   p_bmp180->cal_param.ac5 =  (vdata[8] << 8) | vdata[9];
   p_bmp180->cal_param.ac6 = (vdata[10] << 8) | vdata[11];

   /*parameters B1,B2*/
   p_bmp180->cal_param.b1 =  (vdata[12] << 8) | vdata[13];
   p_bmp180->cal_param.b2 =  (vdata[14] << 8) | vdata[15];

   /*parameters MB,MC,MD*/
   p_bmp180->cal_param.mb =  (vdata[16] << 8) | vdata[17];
   p_bmp180->cal_param.mc =  (vdata[18] << 8) | vdata[19];
   p_bmp180->cal_param.md =  (vdata[20] << 8) | vdata[21];
   return comres;
}
...
unsigned short bmp180_get_ut()
{
   ...
   comres += p_bmp180->BMP180_BUS_READ_FUNC(p_bmp180->dev_addr,\
		BMP180_ADC_OUT_MSB_REG, vdata, 2);
   ut = (vdata[0] << 8) | vdata[1];
   return ut;
}
   ...
   int comres = 0;
	if (p_bmp180->chip_id == BMP180_CHIP_ID && p_bmp180->sw_oss == 1 && \
		...
		for (j = 0 ; j < 3; j++) {
				...
				vdata, 3);
				sum = (((unsigned long) vdata[0] << 16) |\
				((unsigned long) vdata[1] << 8) |\
				(unsigned long) vdata[2]) >> \
	...
	if (p_bmp180->chip_id == BMP180_CHIP_ID && p_bmp180->sw_oss == 0) {
		...
		oversampling_setting)));
		comres += p_bmp180->BMP180_BUS_READ_FUNC(p_bmp180->dev_addr,\
		BMP180_ADC_OUT_MSB_REG, vdata, 3);
		up = (((unsigned long) vdata[0] << 16) |\
		((unsigned long) vdata[1] << 8) |\
		(unsigned long) vdata[2]) >> (8-p_bmp180->oversampling_setting);
		p_bmp180->number_of_samples = 1;
	}
    }
return up;
...
void BMP180_delay_msek(unsigned int msek) 		//16
{
