...
//		if(Sever_Order == SERVER_TCPIP)
	...
	//	Uart0_Tx(test1,2);
		...
		//	TCPIP_TcpSend(TcpSocket_ME, forward_buffer, uart1_count-2, TCPIP_SEND_NOT_FINAL);
		 ...
		 if((forward_buffer[6] == 0xff)&&(forward_buffer[6 + 1] == 0x19))			 
		 		length = 15;
		 else if((forward_buffer[6 + 1] == 0x03)&&(TsataId == forward_buffer[6])) 
		 	 length = forward_buffer[6 + 2] + 9 ;		//6head + 3
		 else if((forward_buffer[6 + 1] == 0x06)&&(TsataId == forward_buffer[6]))		
		 	 length = 12;
		 else if((forward_buffer[6 + 1] == 0x10)&&(TsataId == forward_buffer[6]))
			 length	= 12 ;
		 else
		 	 length = 0;

			TCPIP_TcpSend(TcpSocket_ME, forward_buffer, length , TCPIP_SEND_NOT_FINAL);
...
{
	portTickType	xDelayPeriod = ( portTickType ) SYS_FREQUENCY_PERIOD / portTICK_RATE_MS;//200
	portTickType	xDelayPeriod1 = ( portTickType ) SYS_FREQUENCY_PERIOD1 / portTICK_RATE_MS;//100
	...
	{  
          ...
          if(LED==Zigbee_OK) { Led_zigbee_Tx();  vTaskDelay(xDelayPeriod1);	Led_zigbee_Rx(); vTaskDelay(xDelayPeriod1);
							   LED= None;Led_ReSet();	
							 }
...
{
	portTickType xDelayPeriod = ( portTickType ) 50 / portTICK_RATE_MS;//50
...
//	Uart0_Tx(test,5);	//for test power delay
